	...
	*/
	void CutOut(SimpleBitmapTemplate <ComponentType, NumComponentPerPixel> &destination, int thisX0, int thisY0, int wid, int hei, ComponentType clearColor) const
	{
		...
		for (row = 0; row < hei; row++)
		{
			...
			{
				int colsum = thisX0 + col;
				int rowsum = thisY0 + row;
				if (IsInRange(colsum, rowsum))
				{
					destination.bmpPtr[(row * wid + col) * NumComponentPerPixel ] = bmpPtr[rowsum * GetNumComponentPerLine() + colsum * NumComponentPerPixel];
					destination.bmpPtr[(row * wid + col) * NumComponentPerPixel + 1] = bmpPtr[rowsum * GetNumComponentPerLine() + colsum * NumComponentPerPixel + 1];
					destination.bmpPtr[(row * wid + col) * NumComponentPerPixel + 2] = bmpPtr[rowsum * GetNumComponentPerLine() + colsum * NumComponentPerPixel + 2];
					destination.bmpPtr[(row * wid + col) * NumComponentPerPixel + 3] = bmpPtr[rowsum * GetNumComponentPerLine() + colsum * NumComponentPerPixel + 3];
				}
				else
				{
					destination.bmpPtr[(row * wid + col) * NumComponentPerPixel] = clearColor;
					destination.bmpPtr[(row * wid + col) * NumComponentPerPixel + 1] = clearColor;
					destination.bmpPtr[(row * wid + col) * NumComponentPerPixel + 2] = clearColor;
					destination.bmpPtr[(row * wid + col) * NumComponentPerPixel + 3] = clearColor;
				}
			}
		}
	}
