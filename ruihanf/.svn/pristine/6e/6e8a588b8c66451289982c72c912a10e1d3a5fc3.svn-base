...
protected:
	...
	{
		while (GetNode(ndHd) != nullptr) {
			NodeHandle left = Left(ndHd);
			NodeHandle right = Right(ndHd);
			NodeHandle left_left = Left(left);
			NodeHandle left_right = Right(left);
			NodeHandle right_left = Left(right);
			NodeHandle right_right = Right(right);
			//Left Case
			if (GetHeight(left) - GetHeight(right) >= 2)
			{
				if (GetHeight(left_left) - GetHeight(left_right) <= -1)
				{
					RotateLeft(left);
				}
				RotateRight(ndHd);
			...
			if (GetHeight(left) - GetHeight(right) <= -2)
			{
				if (GetHeight(right_left) - GetHeight(right_right) >= 1)
				{
					RotateRight(right);
				}
				RotateLeft(ndHd);
...
public:
	bool Delete(NodeHandle ndHd)
	{
		...
		else if(ndHd.IsNotNull())
		{
			...
			if(true==SimpleDetach(RMOL))
			{
				...
				if (autoRebalance)
				{
					Rebalance(MakeHandle(upOfRMOLptr));
				}
				--nElem;
				return true;
			}
	...
	void TreeToVine() 
	{
		NodeHandle ndHd = RootNode();
		while (ndHd != nullptr) {
			if (nullptr != GetNode(Left(ndHd))) {
				RotateRight(ndHd);
				ndHd = Up(ndHd);
			}
			else 
	...
	{
		NodeHandle ndHd = RootNode();
		while (nullptr != GetNode(ndHd) && n > 0) 
		{
			if (nullptr!=GetNode(Right(ndHd)))
			{
				RotateLeft(ndHd);
				ndHd=Right(Up(ndHd));
			}
